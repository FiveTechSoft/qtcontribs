...
METHOD IdeSelectSource:loadSources( cPathFile )
   ...
   WITH OBJECT ::oUI
      ...
      :comboSelect:clear()
      :comboUnSelect:clear()
      :treeSources:headerItem():setText( 0, "..." )
   ENDWITH
   ::aExt := {}
...
METHOD IdeSelectSource:buildUI()
   WITH OBJECT ::oUI := hbide_getUI( "SelectSources" )
      ...
      ENDWITH
      WITH OBJECT :btnHbp
         :setIcon( QIcon( hbide_image( "folder" ) ) )
         :connect( "clicked()", {|| hbide_fetchProject( :editHbp ) } )
      ENDWITH 
      WITH OBJECT :editHbp
         :connect( "textChanged(QString)", {|cPath| ::loadSources( cPath ) } )
         :setText( hbide_getNextProject( hbide_setWorkingProjectFolder() ) )
      ENDWITH 
   ENDWITH
   RETURN Self
...
METHOD IdeProjManager:destroy()
   IF !empty( ::oUI )
      WITH OBJECT ::oUI
         : buttonCn          : disconnect( "clicked()" )
         : buttonSave        : disconnect( "clicked()" )
         : buttonSaveExit    : disconnect( "clicked()" )
         : buttonSelect      : disconnect( "clicked()" )
         : buttonUp          : disconnect( "clicked()" )
         : buttonDown        : disconnect( "clicked()" )
      // : buttonSort        : disconnect( "clicked()" )
      // : buttonSortZA      : disconnect( "clicked()" )
      // : buttonSortOrg     : disconnect( "clicked()" )
      // : tabWidget         : disconnect( "currentChanged(int)" )
         : buttonChoosePrjLoc: disconnect( "clicked()" )
         : buttonChooseWd    : disconnect( "clicked()" )
         : buttonChooseDest  : disconnect( "clicked()" )
         : buttonBackup      : disconnect( "clicked()" )
         : editPrjLoctn      : disconnect( "textChanged(QString)" )
         : destroy()           
      ENDWITH
   ENDIF
   IF !empty( ::aPrjProps )
...
METHOD IdeProjManager:buildInterface()
   ...
   WITH OBJECT ::oUI := hbide_getUI( "projectpropertiesex" )
      ::oPropertiesDock:oWidget:setWidget( :oWidget )
      WITH OBJECT :comboPrjType
         :addItem( "Executable" )
         :addItem( "Library"    )
         :addItem( "Dll"        )
      ENDWITH 
...
METHOD IdeProjManager:buildSource( lExecutable )
   ...
   WITH OBJECT ::oProcess := HbpProcess():new()
      ...
      :finished    := {|nEC , nES, oHbp| ::finished( nEC, nES, oHbp, ::cWrkEnvironment ) }
      :workingPath := hbide_pathToOSPath( oEdit:cPath )
   ENDWITH 
...
METHOD IdeProjManager:buildProject( cProject, lLaunch, lRebuild, lPPO, lViaQt, cWrkEnviron, lDebug )
   LOCAL cHbpPath, oEdit, cHbpFN, cTmp, cExeHbMk2, aHbp, cCmd, cC
   LOCAL oProcess, cBatch, oSource, cCmdParams, cBuf, n, aHbpData
   LOCAL cbRed := "<font color=blue>", ceRed := "</font>"
   IF .T.
   ...
   ELSE
      ...
      ::outputText( cTmp )
      ::outputText( hbide_outputLine() )
      WITH OBJECT ::oIDE:oEV := IdeEnvironments():new():create( ::oIDE )
      ...
      WITH OBJECT oProcess := HbpProcess():new()
         :output      := {|cOut, mp2, oHbp| ::showOutput( cOut, mp2, oHbp ) }
         :finished    := {|nEC , nES, oHbp| FErase( cBatch ), ::finished( nEC, nES, oHbp, cWrkEnviron, lDebug ) }
         :workingPath := hbide_pathToOSPath( ::oProject:location )
         //
         cCmd := hbide_getShellCommand()
         cC   := iif( hbide_getOS() == "nix", "", "/C " )
         IF hb_fileExists( cBatch )
            cBuf := hb_memoread( cBatch )
