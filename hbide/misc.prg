...
FUNCTION hbide_execPopup( aPops, aqPos, qParent )
   ...
   WITH OBJECT qPop := QMenu( iif( HB_ISOBJECT( qParent ), qParent, NIL ) )
      :setStyleSheet( GetStyleSheet( "QMenuPop", hbide_setIde():nAnimantionMode ) )
   ENDWITH 
...
FUNCTION hbide_showWarning( cMsg, cInfo, cTitle, qParent )
   ...
   ENDIF
   WITH OBJECT oMB := QMessageBox( qParent )
      :setText( cMsg )
      IF !empty( cInfo )
         :setInformativeText( cInfo )
      ENDIF
      :setIcon( QMessageBox_Critical )
...
FUNCTION hbide_getYesNoCancel( cMsg, cInfo, cTitle )
   ...
   IF .T.
      DEFAULT cTitle TO "Option Please!"
   ENDIF
   WITH OBJECT oMB := QMessageBox( SetAppWindow():oWidget )
      :setText( "<b>"+ cMsg +"</b>" )
      IF !empty( cInfo )
         :setInformativeText( cInfo )
      ENDIF
...
FUNCTION hbide_fetchAFile( oWnd, cTitle, aFlt, cDftDir, cDftSuffix, lAllowMulti )
   ...
   ENDIF
   WITH OBJECT oDlg := XbpFileDialog():new():create( oWnd, , { 10,10 } )
      :title       := cTitle
      :center      := .t.
      :fileFilters := aFlt
      IF HB_ISSTRING( cDftSuffix )
         :oWidget:setDefaultSuffix( cDftSuffix )
      ENDIF
      :oWidget:setOption( QFileDialog_DontUseNativeDialog, .T. )
   ENDWITH 
   RETURN oDlg:open( cDftDir, NIL, lAllowMulti )
...
FUNCTION hbide_fetchADir( oWnd, cTitle, cDftDir )
   ...
   WITH OBJECT oDlg := XbpFileDialog():new():create( oWnd, , { 10,10 } )
      :title  := cTitle
      :center := .t.
      :oWidget:setFileMode( 4 )
      cFile := oDlg:open( cDftDir, NIL, .f. )
      :destroy()
...
FUNCTION hbide_readSource( cTxtFile )
   RETURN hb_ATokens( StrTran( hb_MemoRead( cTxtFile ), Chr( 13 ) ), Chr( 10 ) )
...
FUNCTION hbide_fetchADate( qParent, cTitle, cPrompt, dDefault )
   ...
   ENDIF
   WITH OBJECT oUI := hbide_getUI( "fetchdate", qParent )
      :setWindowTitle( cTitle )
      :labelPrompt:setText( cPrompt )
      IF dDefault != NIL
         WITH OBJECT qDate := QDate()
            :setYear( year( dDefault ) )
            :setMonth( month( dDefault ) )
            :setDay( day( dDefault ) )
         ENDWITH 
         oUI:editDate:setDate( qDate )
      ENDIF
      :buttonOk    : connect( "clicked()", {|| oUI:done( 1 ) } )
      :buttonCancel: connect( "clicked()", {|| oUI:done( 0 ) } )
      nRet := :exec()
...
FUNCTION hbide_fetchAString( qParent, cDefault, cWhat, cTitle )
   ...
   ENDIF
   WITH OBJECT qGo := QInputDialog( qParent )
      :setTextValue( cDefault )
      :setLabelText( cWhat )
      :setWindowTitle( cTitle )
      :exec()
      cText := qGo:textValue()
...
FUNCTION hbide_getFuncObjectFromHash( hDoc )
   LOCAL oFunc := IdeDocFunction():new()
   IF "TEMPLATE" $ hDoc
...
FUNCTION hbide_fetchASelection( aList )
   ...
   WITH OBJECT oSL := hbide_getUI( "selectionlist", hbide_setIde():oDlg:oWidget )
      :setWindowTitle( "Select an Item" )
      :listOptions :connect( "doubleClicked(QModelIndex)", {|p| selectionProc( 1, p, @cChoice, aList, oSL ) } )
      :buttonOk    :connect( "clicked()"                 , {|p| selectionProc( 2, p, @cChoice, aList, oSL ) } )
      :buttonCancel:connect( "clicked()"                 , {|p| selectionProc( 3, p, @cChoice, aList, oSL ) } )
   ENDWITH
