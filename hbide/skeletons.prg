...
METHOD IdeSkeletons:destroy()
   IF !empty( ::oUI )
      WITH OBJECT ::oUI
         :buttonNew   :disconnect( "clicked()" )
         :buttonRename:disconnect( "clicked()" )
         :buttonDelete:disconnect( "clicked()" )
         :buttonClear :disconnect( "clicked()" )
         :buttonGetSel:disconnect( "clicked()" )
...
METHOD IdeSkeletons:show()
   IF empty( ::oUI )
      WITH OBJECT ::oUI := hbide_getUI( "skeletons" )
         ::oSkeltnDock:oWidget:setWidget( :oWidget )
         //
         :buttonNew   :connect( "clicked()"             , {|| ::execEvent( __buttonNew_clicked__              ) } )
         :buttonRename:connect( "clicked()"             , {|| ::execEvent( __buttonRename_clicked__           ) } )
         ...
         :buttonGetSel:connect( "clicked()"             , {|| ::execEvent( __buttonGetSel_clicked__           ) } )
         ...
         //:editCode:setFontFamily( "Courier New" )
         //:editCode:setFontPointSize( 10 )
         :editCode:setFont( ::oFont:oWidget )
      ENDWITH 
   ENDIF
   ::refreshList()
   RETURN Self
...
METHOD IdeSkeletons:postText( qEdit, cText )
   ...
   WITH OBJECT qCursor
      ...
      IF !empty( nColCur )
         ...
         :movePosition( QTextCursor_Right, QTextCursor_MoveAnchor, nCol + nColCur - 1 )
      ENDIF
   ENDWITH 
   qEdit:setTextCursor( qCursor )
   RETURN Self
...
METHOD IdeSkeletons:showTree()
   IF empty( ::oTree )
      WITH OBJECT ::oTree := XbpTreeView():new()
         :hasLines   := .T.
         :hasButtons := .T.
         :create( ::oSkltnsTreeDock, , { 0,0 }, { 10,10 }, , .t. )
         WITH OBJECT :oWidget
            :setMinimumWidth( 100 )
            ...
            :setIndentation( 12 )
         ENDWITH
         :itemSelected  := {|oItem         | ::execEvent( __oTree_itemSelected__, oItem ) }
         :hbContextMenu := {|mp1, mp2, oXbp| ::execEvent( __oTree_contextMenu__ , { mp1, mp2, oXbp } ) }
      ENDWITH 
